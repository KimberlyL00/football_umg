...
<%
    ...
    if (nombre != null && !nombre.trim().isEmpty() && ciudad != null && !ciudad.trim().isEmpty()) {
        ...
        e.setCiudad(ciudad.trim());
        new EstadioDAO().insertar(e);
...
<head>
    <meta charset="UTF-8">
    <title>Agregar Estadio | Mundial 2026</title>
    <style>
...
<div class="container">
    ...
    <form method="post" action="agregar_estadio.jsp">
        <label>Nombre del Estadio</label>
        <input type="text" name="nombre" placeholder="Ej: Estadio Azteca" required />
        <label>Ciudad</label>
        <input type="text" name="ciudad" placeholder="Ej: Ciudad de Mexico" required />
        <input type="submit" class="btn-submit" value="Guardar Estadio" />
    </form>
    <a href="gestion_estadios.jsp" class="back">← Volver</a>
</div>
</body>
</html>