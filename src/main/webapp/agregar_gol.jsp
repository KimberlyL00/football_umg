...
<%
    ...
    String tipoGol = request.getParameter("tipo_gol");

    if (idPartidoStr != null && idJugadorStr != null && minutoStr != null && tipoGol != null) {
        Gol g = new Gol();
        g.setId_partido(Integer.parseInt(idPartidoStr));
        g.setId_jugador(Integer.parseInt(idJugadorStr));
        g.setMinuto(Integer.parseInt(minutoStr));
        g.setTipo_gol(tipoGol);
        new GolDAO().insertar(g);
...
<head>
    <meta charset="UTF-8">
    <title>Registrar Gol | Mundial 2026</title>
    <style>
...
<div class="container">
    <h1>Registrar Gol</h1>
    <form method="post" action="agregar_gol.jsp">
        <label>Partido</label>
        ...
        <label>Minuto</label>
        <input type="number" name="minuto" placeholder="Ej: 45" min="1" max="120" required />

        <label>Tipo de Gol</label>
        <select name="tipo_gol" required>
        ...
        <input type="submit" class="btn-submit" value="Registrar Gol" />
    </form>
    <a href="gestion_goles.jsp" class="back">← Volver</a>
</div>
</body>
</html>