...
<head>
    <meta charset="UTF-8">
    <title>Posiciones | Mundial 2026</title>
    <style>
...
<div class="container">
    <h1>Posiciones</h1>
    <div class="nav-bar">
        <a href="agregar_posicion.jsp" class="btn-add">+ Agregar Posicion</a>
        ...
        <tbody>
            <%
                ...
                try {
                    ...
                    PreparedStatement ps = conn.prepareStatement("SELECT id_posicion, descripcion FROM posicion ORDER BY id_posicion ASC");
                    ResultSet rs = ps.executeQuery();
                    while (rs.next()) {
            %>
            <tr>
                <td style="color: var(--accent);">#<%= rs.getInt("id_posicion") %></td>
                <td><%= rs.getString("descripcion") %></td>
                <td>
                    <a href="eliminar_posicion.jsp?id=<%= rs.getInt("id_posicion") %>" class="btn-del"
                       onclick="return confirm('¿Eliminar esta posicion?')">Eliminar</a>
                </td>
            </tr>
            <%
                    }
            ...
            <tr>
                <td colspan="3">
                    <div class="error-msg">Error al cargar posiciones: <%= e.getMessage() %></div>
                </td>
            </tr>
