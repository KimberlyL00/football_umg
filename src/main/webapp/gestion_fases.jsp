...
<head>
    <meta charset="UTF-8">
    <title>Fases | Mundial 2026</title>
    <style>
...
<div class="container">
    <h1>Fases</h1>
    <div class="nav-bar">
        <a href="agregar_fase.jsp" class="btn-add">+ Agregar Fase</a>
        ...
        <tbody>
            <%
                ...
                try {
                    ...
                    PreparedStatement ps = conn.prepareStatement("SELECT id_fase, descripcion FROM fase ORDER BY id_fase ASC");
                    ResultSet rs = ps.executeQuery();
                    while (rs.next()) {
            %>
            <tr>
                <td style="color: var(--accent);">#<%= rs.getInt("id_fase") %></td>
                <td><%= rs.getString("descripcion") %></td>
                <td>
                    <a href="eliminar_fase.jsp?id=<%= rs.getInt("id_fase") %>" class="btn-del"
                       onclick="return confirm('¿Eliminar esta fase?')">Eliminar</a>
                </td>
            </tr>
            <%
                    }
            ...
            <tr>
                <td colspan="3">
                    <div class="error-msg">Error al cargar fases: <%= e.getMessage() %></div>
                </td>
            </tr>
